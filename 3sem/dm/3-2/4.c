...
void relationUnionWithPair(int A[SIZE][SIZE], int x, int y) {
	A[x][y] = 1;
}
...
int transitiveClosure(int A[SIZE][SIZE], int C[SIZE][SIZE], int* comparisons) {
    ...
    // компилятор ругается на название матрицы C2, использую S
    // int S[SIZE][SIZE];
    // int compositions = 0;
    // copyMatrix(A, C);
